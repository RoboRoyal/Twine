...
bool TransBlock(Block *B) {
    debugT("TransBlock()");
    if (B == NULL || B->Lines.size() == 0) {
        debugT("TransBlock()EMPTY done");
        return true;
    }
...
bool TransFOR_IN(FOR_IN *N) {
    debugT("TransFOR_IN()");
    if (N->vectorRange) {
        string vName = N->itteratorName +
                       "_IT";//TODO i would actually like to be able to do this with not just vectors, but also for in to other types
        ...
        out += ".ptr)->begin(); " + vName + " != ((vector<__ANY__>*)" + N->varName + ".ptr)->end(); " + vName +
               "++){__ANY__ " + N->itteratorName + "=* " + vName + ";";
    } else {
        out += "for(int " + N->itteratorName + " = " + N->start + "; " + N->itteratorName + " != " + N->end + "); " +
               N->itteratorName + "++){";
    }
    TransBlock(N->blockState);
    out += "}/*end FOR_IN*/\n";
