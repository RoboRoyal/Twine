...
void parseNormalFOR(FOR2 * F){//TODO this is broken
    ...
    F->lineOne = me;
    parseLine(F->lineOne);
    expect(";");//is this what i want to put between statements?
...
void parseIdent(parseNode * pn){//TODO add support for x++
    ...
    if(peek("(") && isCallable(lastSym->tokenText)){//&&expect("(")){//if you want to allow vars with same name as functions, change this
        ...
        if(isConstant(varName) && isLocal(varName)) {//TODO add warning about shadow var(vars with same name in different scope)
            error("Trying to set constant variable '" + varName + "' to a new value", true);
        }
        pn->type = "varAss";
        ...
        if(getVARType(varName) == "DNE" || isConstant(varName)){//if it is constant, that means a new var must be made to shadow the constant var
            if(isConstant(varName))
                warn("Variable '"+varName+"' is shadowing existing constant variable with same name");
            //setVar(varName);
            me->newVar = true;
            needToSetVar = true;
            tVar->type = "__ANY__";
...
bool expect(const string& s,bool allowForce){
    if(accept(s))
        return true;
    if(FORCE && allowForce){
        forceAdd(s);
...
void forceAdd(const string& newSym){//TODO
    dummy();//fix up etc
...
void error(const string& msg, bool passable){
    ...
    errors++;//note how many errors we find

    //see if we should color the error message at all
    string errorMsg;
    ...
    //note the current function, class, and file
    if(!(currentFunk->name == "__userMain__" && currentClass != NULL))//if were in a class but not a function, don't say were in function '__userMain__'
    ...
    //see if we should note error and continue, or error out
    if(!passable){
